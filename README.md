...
## CRM & Admin Backend — Supabase Secrets

The `/admin` application uses Supabase Edge Functions for AI-powered lead discovery and enrichment.
The following secrets must be set in your Supabase project before deploying the functions:

```bash
# Required
supabase secrets set GEMINI_API_KEY="your-google-ai-api-key"

# Optional but recommended — Groq (fast, free-tier 100k tokens/day)
supabase secrets set GROQ_API_KEY="your-groq-api-key"

# Optional but recommended — OpenRouter (aggregates dozens of free AI models)
...
# Google Places API (needed for Groq + Places engine)
supabase secrets set GOOGLE_PLACES_API_KEY="your-google-maps-api-key"
```

### Deploying Edge Functions

```bash
supabase functions deploy autonomous-web-sniper
supabase functions deploy auto-enrich-leads
supabase functions deploy enrich-imported-leads
supabase functions deploy email-reply-webhook
```

### AI Engine Configuration

All three AI engines (Gemini, Groq, OpenRouter) can be independently enabled/disabled from the
Admin UI at `/admin/emaily/sber`. When multiple engines are active they run **in parallel**:
- **Discovery (Sběr):** Each enabled engine searches independently; results are merged and deduplicated by email.
- **Enrichment:** Each enabled engine enriches the same batch; their outputs are deep-merged (later engines' fields override earlier ones).

| Engine | Free-tier limit | Speed | Notes |
|--------|----------------|-------|-------|
| Gemini 2.5 Flash | ~1 500 req/day, ~4M tokens/day | Medium | Google Search grounding — best quality |
| Groq LLaMA 3.3 70B | 100 000 tokens/day | Fast | No web search; uses Google Places API for discovery |
| OpenRouter (meta-llama/llama-3.3-70b-instruct:free) | Varies by model | Medium | Aggregator — no credit card required |
