...
INSERT INTO service_categories (id, name, icon, slug) VALUES
  ...
  (gen_random_uuid(), 'Rekonstrukce', 'Building', 'rekonstrukce'),
...
CREATE POLICY "Customers can create offers for their jobs"
ON public.offers
FOR INSERT
TO authenticated
WITH CHECK (
...
-- Create analytics_events table for tracking sessions, conversions, searches, page views
...
-- We also need to add a variant tracking column to email_logs if it doesn't exist already.
